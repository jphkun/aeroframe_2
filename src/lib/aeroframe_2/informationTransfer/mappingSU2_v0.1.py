...
class mapper:
    ...
    def computesTransformationsMatrices(self,forceFilePath):
        ...
        self.iM = np.linalg.inv(Mbeam)
        logger.debug('shape of iM: ' + str(self.iM.shape))
        
        # Computes the matrix Q which is also the matrix A transposed in
        # this specific case.
        Y = SU2Data.iloc[:,1:4].values
        logger.debug(Y)
        r = sp.spatial.distance.cdist(X,Y,"euclidean")
        # Computes the radial basis function matrix "Q" in the report
        Q = self.phi(r,fun)
        self.A = Q.T
        self.H = np.matmul(self.A,self.iM)
        logger.debug("shape of A:  "+str(self.A.shape))
        logger.debug("shape of iM: "+str(self.iM.shape))
        logger.debug("shape of H:  "+str(self.H.shape))
    ...
    def aeroToStructure(self,forceFilePath):
        ...
        # Regroups all the forces and moments so they can be multiplied
        # by the matrix
        AFX = np.hstack(self.afx)
        AFY = np.hstack(self.afy)
        AFZ = np.hstack(self.afz)
        AMX = np.hstack(self.amx)
        AMY = np.hstack(self.amy)
        AMZ = np.hstack(self.amz)
        # logger.debug('AFX: ' + str(AFX.shape))

        # Computes the forces
        SFX = np.matmul(self.H.T, AFX)
        SFY = np.matmul(self.H.T, AFY)
        SFZ = np.matmul(self.H.T, AFZ)
        # Computes the moments
        SMX = np.matmul(self.H.T, AMX)
        SMY = np.matmul(self.H.T, AMY)
        SMZ = np.matmul(self.H.T, AMZ)
        logger.debug('AFX: ' + str(SFX.shape))
        
        # Separates the struture forces back to the initial assigned point.
        N = len(self.geoP)
        k = 0
        for i in range(N):
            M = len(self.geoP[i])
            # for j in range(M):
            self.sfx.append(AFX[k:k+M])
            self.sfy.append(AFY[k:k+M])
            self.sfz.append(AFZ[k:k+M])
            self.smx.append(AMX[k:k+M])
            self.smy.append(AMY[k:k+M])
            self.smz.append(AMZ[k:k+M])
            logger.debug('' + str(self.sfx[i].shape))
            k += M
        # sys.exit()
        ...
        # Only the force contrbtion related to inertia is taken into account here.
    ...
    def computeDistanceForMorments(self,forceFilePath):
        ...
        6) Select if the current point projection is one of the following
            three cases:
                ...
                3. On no segment and hence the distance is taken from
        ...
        for i in range(N):
            ...
            # Y. This leads to an (NxM) matrix, N being the number of CFD 
            # nodes point and M being the number of structure nodes points.
    ...
    def structureToAero(self,iteration,forceInitFilePath,forceFilePath):
        ...
        # Regroups all beams data
        SUX = np.hstack(self.sux)
        SUY = np.hstack(self.suy)
        SUZ = np.hstack(self.suz)
        STX = np.hstack(self.stx)
        STY = np.hstack(self.sty)
        STZ = np.hstack(self.stz)
        # Computes aircraft surface displacements
        AUX = np.matmul(self.H, SUX)
        AUY = np.matmul(self.H, SUY)
        AUZ = np.matmul(self.H, SUZ)
        ATX = np.matmul(self.H, STX)
        ATY = np.matmul(self.H, STY)
        ATZ = np.matmul(self.H, STZ)
        # TODO find a cleaner way to do this
        self.displacements = np.array([AUX,AUY,AUZ])

        # # Computes the distance between all point in each directions
        # SU2Data = pd.read_csv(forceFilePath)
        # length = len(SU2Data)
        # X = np.vstack((SU2Data.iloc[:,1].values, np.zeros(length))).T.reshape((length,2))
        # Y = np.vstack((SU2Data.iloc[:,2].values, np.zeros(length))).T.reshape((length,2))
        # Z = np.vstack((SU2Data.iloc[:,3].values, np.zeros(length))).T.reshape((length,2))
        # logger.debug(X)
        ########################################################################
        
        # distX = sp.spatial.distance.cdist(X, X, 'euclidean')
        # distY = sp.spatial.distance.cdist(Y, Y, 'euclidean')
        # distZ = sp.spatial.distance.cdist(Z, Z, 'euclidean')
        # logger.debug(ATX)
        # logger.debug(np.max(ATX))
        # logger.debug(np.min(ATX))
        # logger.debug(np.min(np.abs(ATX)))
        # logger.debug(distX * X)
        # sys.exit()
        ...
        # idx = str(SU2DataInit.iloc[:,0].values.T)
        # logger.debug(len(idx))
        ...
        #     ax.scatter(SU2DataInit['x'],
        #                SU2DataInit['y'],
        #                SU2DataInit['z'],
        #                label='undeformed')

        #     ax.scatter(SU2DataInit['x'] + self.displacements[0,:],
        #                SU2DataInit['y'] + self.displacements[1,:],
        #                SU2DataInit['z'] + self.displacements[2,:],
        #                label='deformed')
        #     val = 15
        #     ax.set_xlim(-val,val)
        #     ax.set_ylim(-val,val)
