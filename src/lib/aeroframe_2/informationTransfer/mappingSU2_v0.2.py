...
class mapper:
    ...
    def structureToAero(self,iteration,forceInitFilePath,forceFilePath):
        ...
        self.findCloseToFuselage(forceFilePath)
        ...
        if self.plotting:
            ...
            ax.scatter(self.fuselagePointsCTW[:,0],
                       self.fuselagePointsCTW[:,1],
                       self.fuselagePointsCTW[:,2],
                       label='undeformed')

            ax.scatter(SU2Data['x'],
                       SU2Data['y'],
                       SU2Data['z'],
                       label='undeformed')
            # ax.scatter(SU2Data['x'] + self.displacements[:,0],
            #            SU2Data['y'] + self.displacements[:,1],
            #            SU2Data['z'] + self.displacements[:,2],
            #            label='deformed')
            val = 15
            ax.set_xlim(-val+10,val+10)
            ax.set_ylim(-val,val)
            ax.set_zlim(-val+4,val+4)
            ax.legend()
            plt.show()
        del(SU2Data)

    def findCloseToFuselage(self,forceFilePath):
        """
        ...
        distanceToWing = np.min(distance,axis=1)
        ##
        distanceToFuselage = np.min(distance,axis=0)
        logger.debug(distanceToFuselage.shape)
        sys.exit()
        # Arbitrary value that selects the fuselage points that are new the
        # wing.
        distanceMax = 0.2
        
        # Fuselage point indexes selected with this criteras
        indexesFuselageTF = np.where(np.logical_and(distanceToWing>=0, distanceToWing<distanceMax))
        indexesFuselage = aircraftPointsAndForcesCFD[0][indexesFuselageTF,0]
        indexesFuselage = indexesFuselage[0]
        # distance matrix that constains only the selected n fuselage points
        # and the M wing points
        selectedDistance = distance[indexesFuselageTF]
        # logger.debug(selectedDistance.shape)
        # logger.debug(fuselagePoints)
        
        
        # Point at which we should add the new displacement
        modifiedFulseagePointsIndex = SU2Data.loc[indexesFuselage]['ids'].values
        logger.debug(modifiedFulseagePointsIndex)
        # # Old displacement of these fuselage points
        # logger.debug(self.displacements[indexesFuselageTF])

        # Finds the related wing point index that is closer to the "current"
        # fuselage point
        logger.debug(selectedDistance.shape)
        localWingPointIndex = np.argmin(selectedDistance,axis=1)
        localWingPointDist = np.min(selectedDistance,axis=1)
        logger.debug(localWingPointIndex)
        # Finds the global point index
        globalWingPointIndex = aircraftPointsAndForcesCFD[1][localWingPointIndex,0]
    ...
    def polynome(self,x):
        ...
        ----------
        x : float
            distance between the fuselage point and the closest wing point
