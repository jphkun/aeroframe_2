...
cwd1 = '/home/cfse2/Documents/aeroframe_2/test/static/8_SweptWingValidationPytornadoFramAT_case1/CFD/_results/'
cwd2 = '/home/cfse2/Documents/aeroframe_2/test/static/8_SweptWingValidationPytornadoFramAT_case1/'
...
df1['x_my'] = df1['x']
# aerodynamic moment
df1['My'] = (-df1['x'] + ((a-0.5) * 2) + np.abs((df1['y'] / np.cos(np.pi / 4))))* (df1['Fz']/np.cos(np.pi/4))
...
c1 = -((cFz[6]*l**7)/7 +  (cFz[4]*l**5)/5  + (cFz[2]*l**3)/3 +  ((cFz[0] - n*linearMass*9.81)*l**1)/1)
c2 = -((cFz[6]*l**8)/56 + (cFz[4]*l**6)/30 + (cFz[2]*l**4)/12 + ((cFz[0] - n*linearMass*9.81)*l**2)/2 +c1*l)
# print(cFz)
...
plt.plot(y_cfd,w(np.abs(y_cfd/np.cos(np.pi/4))),'-',label='Analytical solution', linewidth=width)
errorMax = 100*(np.max(np.abs((df3['dz']))) - np.max(np.abs(w(np.abs(y_cfd/np.cos(np.pi/4)))))) / np.max(np.abs(w(np.abs(y_cfd/np.cos(np.pi/4)))))
errorMax = errorMax.round(decimals=1)
# print(np.max(np.abs((df3['dz']))))
# print(np.max(np.abs(w(np.abs(y_cfd)))))
# print(errorMax)
# sys.exit()
