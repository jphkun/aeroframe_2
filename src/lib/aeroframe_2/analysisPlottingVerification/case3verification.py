...
cwd1 = '/home/cfse2/Documents/aeroframe_2/test/static/3_WingValidationPytornadoFramAT_case3/CFD/_results/'
cwd2 = '/home/cfse2/Documents/aeroframe_2/test/static/3_WingValidationPytornadoFramAT_case3/'
...
errorMax = 100*(np.max(np.abs(df3['dz'])) - np.max(np.abs(w(np.abs(y_cfd))))) / np.max(np.abs(w(np.abs(y_cfd))))
...
errorMax2 = 100*(np.max(np.abs(df3['tx'])) - np.max(np.abs(t(np.abs(y_cfd))))) / np.max(np.abs(t(np.abs(y_cfd))))
